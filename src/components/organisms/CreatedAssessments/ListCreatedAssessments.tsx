...
import { FETCH_CREATED } from 'src/lib/graphql/Query'
...
function ListCreatedAssessments() {
  ...
  const auth = useAuth()
  const [getData, { loading, error }] = useLazyQuery(FETCH_CREATED)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setCreatedAssessments(result.data.createdAssessments)
    }
  ...
  return (
    <Grid container spacing={6}>
      <TableAllAssessments data={createdAssessments} />
    </Grid>
  )
}

export default ListCreatedAssessments
